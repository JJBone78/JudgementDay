...
public class Unit : RTSObject, IOrderable
{
    ...
    protected void Update()
    {
        ...
        if (_clicked)
        {
            if (!UIManager.main.IsShiftDown)
                selectedManager.DeselectAll();
            selectedManager.AddObject(this);
            _clicked = false;
        }
    }
    ...
    public void GiveOrder(Order order)
    {
        switch (order.OrderType)
        {
            ...
            case Const.ORDER_LIGHTS_ON:
                GetComponent<Lighting>().CommandLighting(order.OrderType);
                break;
            case Const.ORDER_LIGHTS_AUTO:
                GetComponent<Lighting>().CommandLighting(order.OrderType);
                break;
            case Const.ORDER_LIGHTS_OFF:
                GetComponent<Lighting>().CommandLighting(order.OrderType);
                break;
        }
    }
